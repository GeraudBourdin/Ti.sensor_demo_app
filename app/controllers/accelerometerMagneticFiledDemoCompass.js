...
var sensorsCallback = function(e) {
	if (e.sType == sensor.TYPE_MAGNETIC_FIELD) {
		...
		if(lastTime + offset < now){
			$.degrees.text = e.compassRotation;
			//img.transform = Titanium.UI.create2DMatrix().rotate( -e.compassRotation );
			var rotation = Titanium.UI.create2DMatrix().rotate( -e.compassRotation );
